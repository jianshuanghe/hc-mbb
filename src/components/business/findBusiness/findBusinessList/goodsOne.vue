  ...
  import { mapMutations, mapGetters } from 'vuex';
    export default {
        ...
        watch: {
          LEAVEMESSAGE: {
            handler (a, b) {
              console.log(a, b, 'vuex数据goodsOne');
              console.log(Number(this.items.instId), Number(a.instrucId), '数据goodsOne');
        ...
        created () {
          ...
          console.log(this.msgData, '子组件goodsOne获得数据');
          let address = '';
            ...
            if (localStorage.getItem('clickRecordsArr')) {
              ...
              this.clickRecordsArr.map((item, index) => {
                if (item.instId === this.msgData.instId) { // 取出缓存中存在的状态
                  ...
                  this.items = item;
                  console.log(this.items, '缓存中的数据赋值');
                }
              });
            }
          },
          setClickRecord (e, items, evn) {
            ...
            if (e === 'find') { // 查看
              ...
              if (localStorage.getItem('clickRecordsArr')) {
                this.clickRecordsArr = JSON.parse(localStorage.getItem('clickRecordsArr')); // 获取缓存中的用户点击行为数组记录
                this.clickRecordsArr.map((item, index) => {
                  if (item.instId === evn.instId) { // 取出缓存中存在的状态
                    this.$store.commit('setLeaveMessageDoc', item.doc); // 更新vuex setLeaveMessageDoc
                  }
                });
              }
