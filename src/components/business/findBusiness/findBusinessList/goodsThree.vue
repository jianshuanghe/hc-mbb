<template>
  <div class="content">
    <div class="goods-box"  @click="clickFindBusinessDetaile(msgData)">
      <div class="" @click="setClickRecord('find', items, msgData)">
        <div class="describe-text left">
          <div class="title-box">
          ...
          <div class="norms">
            <!--<div class="norms-title left">-->
              <!--<p class="">规格</p>-->
            <!--</div>-->
            <!--<div class="norms-text left">-->
              <!--<p class="">产品规格产品规格产品规格产…</p>-->
            <!--</div>-->
            <!--<div class="clear"></div>-->
            <div class="ensure">
        ...
        <div class="describe-img left">
          <img :src="msgData.banner01"  alt="" class="">
        </div>
        <div class="clear"></div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      LEAVEMESSAGE: {
        handler (a, b) {
          console.log(a, b, 'vuex数据goodsThree');
          console.log(Number(this.items.instId), Number(b.instrucId), '数据goodsThree');
    ...
    created () {
      ...
      console.log(this.msgData, '子组件goodsThree获得数据');
      let address = '';
      for (let i in this.msgData.address.split(',')) {
        address += this.msgData.address.split(',')[i];
      };
      this.msgData.address = address;
      // this.msgData.address = this.msgData.address.split(',')[0] + this.msgData.address.split(',')[1] + this.msgData.address.split(',')[2];
    ...
    methods: {
      ...
      setClickRecord (e, items, evn) {
        ...
        if (e === 'find') { // 查看
          ...
          this.clickFindBusinessDetaile(evn);
        } else if (e === 'tel') { // 拨打电话
          this.items.tel = true; // 点击之后状态变化
          this.msgData.telCount = this.msgData.telCount + 1;
      ...
      upDataTel (e) {
        console.log('跟新用户行为数据');
        axios.get(this.api2 + '/rest-rp/instruc/instrucDate?type=8' + '&id=' + e.instId).then((response) => {
          console.log(response);
        }).catch((error) => {
          Toast('网络繁忙，请稍后');
          console.log(error, '网络繁忙，请稍后');
      ...
      clickFindBusinessDetaile (e) {
        console.log(e, '触发进入发现商机详情！');
        if (e.type === '1') {
          let params = {
            href: '', // 用于用户从如何秀返回时使用
            source: 'FB', // 定义来源参数 FB：发现商机页面, FI流量承载页
          ...
          params.href = window.location.href.split('?')[0];
          ...
          if (e.custType === '1') {
            ...
            window.location.href = this.Rhxherf + '/helloWorld?tempType=1' + '&wxId=1.0' + '&source=' + params.source + '&userId=' + params.userId + '&instrucId=' + params.instrucId + '&tempId=' + params.tempId + '&instrucTitle=' + params.instrucTitle + '&agin=200' + '&herf=' + encodeURIComponent(params.href) + '&sessionId=' + params.sessionId + '&like=' + this.items.like;
          } else {
            if (this.Rhxherf === 'http://10.158.41.177:8082') { // 測試環境
              params.tempId = 45;
            }
