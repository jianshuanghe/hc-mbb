    ...
    <div class="main-body" :style="{'-webkit-overflow-scrolling': scrollMode}">
      <v-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore">
        <ul class="list" v-for="(items, index) in pageList" :key="index">
          <li>
            <div class="newsItems-box" @click="goToNewsItemsDetail">
              <div class="time-NT">
                <p class="">2019.04.19 8:36</p>
              </div>
              <div class="NT-content">
                ...
                <div class="introduce-NT">
                  <p class="">代理记账的原则是遵守国家法律法规，符合财务会计制度，保护客户合法权益，保守客户商业秘密。</p>
                </div>
                <div class="line"></div>
...
<script>
  ...
  export default {
    ...
    watch: {
      NEWSCEBNTER: {
        handler (a, b) {
          this.businessData = a.business.newsItemsData; // 更新的数据
          this.pageNum = a.business.pageNum; // 总页数
        },
        deep: true
      }
    },
    ...
    methods: {
      ...mapMutations({
        setNewsCenterSearch1: 'setNewsCenterSearch1',
        setNewsCenterPageNum1: 'setNewsCenterPageNum1',
        setNewsCenterNewsItemsData1: 'setNewsCenterNewsItemsData1',
      ...
      loadPageList () {  // 查询数据
        ...
        axios.post(this.api2 + '/rest-rp/notice', this.searchCondition).then((response) => { // 是否还有下一页，加个方法判断，没有下一页要禁止上拉
          if (response.data.ret === '200') {
            ...
            this.$store.commit('setNewsCenterNewsItemsData1', this.pageList);
      ...
      pageNums (cont) {
        ...
        this.$store.commit('setNewsCenterPageNum1', this.pageNum);
      },
      more () {
        // 分页查询
